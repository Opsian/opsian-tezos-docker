...
RUN git clone --shallow-submodules --recurse-submodules https://github.com/Opsian/opsian-ocaml #
RUN if [ "$ASAN" = "ON" ]; \
...
RUN if [ "$LWT" = "ON" ]; \
      ...
      else opam pin -y --debug -vv add opsian git+file:///opsian-ocaml#main; \
    fi
WORKDIR /tezos
